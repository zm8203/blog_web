  ...
  <gvb_card title="文章推荐">
    <div class="gvb_recommend">
      <div class="item" v-for="item in data.list">
        ...
        <div class="info">
          ...
          <div class="abstract">
            <a-typography-paragraph :ellipsis="{rows: 2}">{{ item.abstract }}</a-typography-paragraph>
          </div>
          <div class="date">
    ...
    <template #head-right>
      <div class="head_right">
        <a :class="{active: params.category === item.value }" href="javascript:void (0)" @click="checkCategory(item)" v-for="item in categoryList">{{ item.label }}</a>
      </div>
    </template>
  </gvb_card>
</template>
...
import {articleCategoryListApi, articleRecommendListApi} from "@/api/article_api";
