<template>
  <div>
    <a-modal title="绑定邮箱" :visible="props.visible" body-class="gvb_bind_email_modal_body" @cancel="close">
      <a-steps v-model:current="step" type="navigation">
        <a-step>输入邮箱</a-step>
        <a-step>输入验证码</a-step>
      </a-steps>
      <a-form ref="formRef" :model="form">
        <a-form-item v-if="step === 1" field="email" label="邮箱"
                     :rules="[{required:true,message:'请输邮箱'}, {type: 'email', message: '邮箱格式错误'}]"
                     :validate-trigger="['blur']">
          <a-input v-model="form.email" placeholder="邮箱"></a-input>
        </a-form-item>
        <a-form-item v-if="step === 2" field="code" label="验证码"
                     :rules="[{required:true,message:'请输验证码'}]"
                     :validate-trigger="['blur']">
          <a-input v-model="form.code" placeholder="验证码"></a-input>
          <template #extra>请输入发送到你邮箱的验证码</template>
        </a-form-item>
        <a-form-item v-if="step === 2" field="password" label="登录密码"
                     :rules="[{required:true,message:'请输登录密码'}]"
                     :validate-trigger="['blur']">
          <a-input v-model="form.password" type="password" placeholder="登录密码"></a-input>
...
import type {userBindEmailType} from "@/api/user_api";
import {Message} from "@arco-design/web-vue";
import {useStore} from "@/stores";
import router from "@/router";
import {userBindEmailApi} from "@/api/user_api";
