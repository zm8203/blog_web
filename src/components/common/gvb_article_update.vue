    ...
    <a-modal
      ...
      <a-form ref="formRef" :model="form" :label-col-props="{span:4}" :wrapper-col-props="{span: 20}">
        <a-form-item field="title" label="文章标题" :rules="[{required:true,message:'请输入文章标题'}]"
          ...
          <a-input ref="titleIptRef" v-model="form.title" placeholder="文章标题"></a-input>
        </a-form-item>
      ...
      <template #footer>
        ...
        <a-button type="primary" v-if="props.type === 'add'" @click="sendArticle" status="danger">直接发布</a-button>
      </template>
    </a-modal>
  </div>
</template>
